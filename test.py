...
while (len(vertical) > 0):
    left = int(centre[0])
    right = int(centre[0])
    vertical_point = int(vertical[0])
    string_data = th1[vertical_point]
    value = string_data[left]
    while (value == 0 and left!=0):
        left-=1
        value = string_data[left]
    left+=1
    value = string_data[right]
    while (value == 0 and right!=width):
        right+=1
        value = string_data[right]
    right-=1
    points.append([left, vertical_point])
    points.append([right, vertical_point])
    vertical.remove(vertical_point)
    cv2.line(th1, (left, vertical_point), (right, vertical_point), (155, 155, 155), 2)
print(points)
